...
ln -s $DIR/sonar/bin/linux-x86-64/sonar.sh /etc/init.d/sonar && sed -i 's|#RUN_AS_USER=.*|RUN_AS_USER=sonar|' /etc/init.d/sonar && \
sed -i 's|#RUN_AS_USER=.*|RUN_AS_USER=sonar|' $DIR/sonar/bin/linux-x86-64/sonar.sh && chkconfig sonar on
$DIR/sonar/bin/linux-x86-64/sonar.sh start
}
